...
async function verificaItens(tam,jid) {
    let itens = await getItens(jid);
    itens = itens.split(', ');
    while (itens.length < tam) {
        const men = {
            text: '🍕 Agora escolha o *Segundo Sabor* do seu Pastel:\n\n',
            buttonText: 'CLIQUE AQUI PARA ESCOLHER',
            footer: '© Pastel do Cabeção 😋🍕',
            sections: sections
        }
        SendMessage(jid, men)
        itens = await getItens(jid);
        itens = itens.split(', ');
    }
	...
	sock.ev.on('messages.upsert', async ({ messages, type }) => {
		...
		if (!msg.key.fromMe && jid !== 'status@broadcast' && !GroupCheck(jid)) {
            ...
            const first_op = [{
                title: '*Mago - R$35,00*',
                rows: [
                   { title:'Queijo' , description: '\nPrimeira Opção', rowId: 'first' },
                   { title:'Presunto' , description: '\nPrimeira Opção', rowId: 'first' },
                   { title:'Carne' , description: '\nPrimeira Opção', rowId: 'first' },
                   { title:'Calabresa' , description: '\nPrimeira Opção', rowId: 'first' },
                ],
             },
            ];
            let second_op = [{
                title: '*Mago - R$35,00*',
                rows: [
                   { title:'Queijo' , description: '\nPrimeira Opção', rowId: 'second', fd:'' },
                   { title:'Presunto' , description: '\nPrimeira Opção', rowId: 'second' },
                   { title:'Carne' , description: '\nPrimeira Opção', rowId: 'second' },
                   { title:'Calabresa' , description: '\nPrimeira Opção', rowId: 'second' },
                ],
             },
				...
				if (msg.message.listResponseMessage.title.includes('Past.')) {
					...
					const sec = [{
						title: '© Pastel do Cabeção 😋🍕', rows: [
							{ title: '👉🏻1 Opção  ', description: '\nR$3,50' },
							{ title: '👉🏻2 Opções ', description: '\nR$4,50' },
							{ title: '👉🏻3 Opções ', description: '\nR$7,00' }
						],
					}]
